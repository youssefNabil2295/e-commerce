...
setsuccess('acount has created')
    ...
    seterrormasge(err?.response?.data?.message || "Registration failed, please try again")
  }
  setloding(false)
}
...
return <>
  <Helmet>
    ...
    <meta name="description" content="Create a new Fresh Cart account to start shopping. Register with your name, phone, email and password." />
  </Helmet>
<div className='w-75 mx-auto py-5 text-start vh-100'>
        ...
        {success ?<div className="alert alert-success p-2 m-1"> {success}</div>: ''}
            ...
            <input className='form-control mt-1 mb-1' placeholder='Enter Phone' type='tel' id='phone' name='phone' onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.phone} />
            {formik.errors.phone &&formik.touched.phone? <div className="alert  p-2 mt-2 alert-danger"> {formik.errors.phone}</div>:''}



            <label htmlFor="email">email : </label>
            <input className='form-control mt-1 mb-1' placeholder='Enter email' type='email' id='email' name='email' onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.email} />
            {formik.errors.email &&formik.touched.email? <div className="alert  p-2 mt-2 alert-danger"> {formik.errors.email}</div>:''}



            <label htmlFor="password">password : </label>
            <input className='form-control mt-1 mb-1' placeholder='Enter Password' type='password' id='password' name='password' onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.password} />
            {formik.errors.password? <div className="alert  p-2 mt-2 alert-danger"> {formik.errors.password}</div>:''}


            <label htmlFor="rePassword">rePassword : </label>
            <input className='form-control mt-1 mb-1' placeholder='Enter RePassword' type='password' id='rePassword' name='rePassword' onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.rePassword} />
